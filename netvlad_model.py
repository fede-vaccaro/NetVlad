...
class NetVLADModel:
    def __init__(self, layer_name='block5_conv2'):
        ...
        self.base_model = Model(vgg_model.input, vgg_model.get_layer('block5_conv2').output)
    ...
    def build_netvladmodel(self, n_classes, kmeans):
        ...
        transpose = Permute((3, 1, 2), input_shape=(-1, 256))(self.base_model([self.images_input]))

        # vgg_output = vgg.output_shape[1]
        reshape = Reshape((256, 14 * 14))(transpose)
        l2normalization = L2NormLayer()(reshape)
        netvlad = NetVLAD(feature_size=14 * 14, max_samples=256, cluster_size=64)(
