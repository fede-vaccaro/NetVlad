...
path_dataset = paths.landmarks_path
dest_path = 'landmark/dataset_clustered_big/'
...
def n_images_in_bound(dir, lower=10, upper=50):
    path_dataset_ = os.path.join(path_dataset, dir)
    imgs = [img for img in os.listdir(path_dataset_)]
    return lower < len(imgs) < upper


# dirs = [dir_ for dir_ in dirs if n_images_in_bound(dir_)]
print("{} label with the given bound.".format(len(dirs)))
