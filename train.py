...
if train:
    ...
    print("Starting Oxford5K mAP: ", np.array(compute_aps(dataset='o', model=vladnet, transform=transform, verbose=True, base_resolution=side_res)).mean())
    # print("Starting Paris6K mAP: ", np.array(compute_aps(dataset='p', model=vladnet, verbose=True)).mean())
    starting_map = hth.tester.test_holidays(model=vladnet, transform=transform, side_res=side_res,
    ...
    for e in range(epochs):
        ...
        val_map = hth.tester.test_holidays(model=vladnet, transform=transform, side_res=side_res,
                                           use_multi_resolution=use_multi_resolution,
                                           rotate_holidays=rotate_holidays, use_power_norm=use_power_norm,
                                           verbose=False)
        ...
        print("Oxford5K mAP: ",
              np.array(compute_aps(dataset='o', model=vladnet, transform=transform, verbose=True, base_resolution=side_res)).mean())
        # print("Paris 6K mAP: ",
